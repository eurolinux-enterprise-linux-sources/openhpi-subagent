 ...
 *
 * $Id: saHpiEventLogInfoTable.c 5452 2006-02-10 19:54:20Z djudkovi $
 *
 *
...
SaErrorT event_log_info_update (SaHpiSessionIdT sessionid)
{
	...
	DEBUGMSGTL ((AGENT, "event_log_info_update called\n"));
        ...
        do { //Get the plugin event log first
                ...
                if (rv != SA_OK) {
                        DEBUGMSGTL ((AGENT, 
                        "event_log_info_update, saHpiRptEntryGet Failed: rv = %d\n",rv));
                        rv =  AGENT_ERR_INTERNAL_ERROR;
                        break;
                }
		...
		rv = saHpiEventLogInfoGet (sessionid, 
                ...
                if (rv == SA_ERR_HPI_CAPABILITY) {
                        DEBUGMSGTL ((AGENT, "SA_ERR_HPI_CAPABILITY: event_log_info_update Failed: rv = %d\n",rv));
                        rv = AGENT_ERR_INTERNAL_ERROR;
                        continue;
                } else if ( (rv != SA_OK) && (rv != SA_ERR_HPI_CAPABILITY) ) {
                        DEBUGMSGTL ((AGENT, "event_log_info_update Failed: rv = %d\n",rv));
                        rv = AGENT_ERR_INTERNAL_ERROR;
                        break;
                }
...
saHpiEventLogInfoTable_cmp( const void *lhs, const void *rhs )
{
         ...
         */
        DEBUGMSGTL ((AGENT, "saHpiEventLogInfoTable_cmp, called\n"));

        /* check for NULL pointers */
        if (lhs == NULL || rhs == NULL ) {
                DEBUGMSGTL((AGENT,"saHpiEventLogInfoTable_cmp() NULL pointer ERROR\n" ));
                return 0;
        }
                         ...
                         saHpiEventLogInfoTable_context * src)
{
   ...
   subagent_lock(&hpi_lock_data);
    ...
    dst->saHpiEventLogInfoEntries = src->saHpiEventLogInfoEntries;

    dst->saHpiEventLogInfoSize = src->saHpiEventLogInfoSize;
    ...
    dst->saHpiEventLogInfoOverflowFlag = src->saHpiEventLogInfoOverflowFlag;

    dst->saHpiEventLogInfoOverflowResetable = src->saHpiEventLogInfoOverflowResetable;

    dst->saHpiEventLogInfoOverflowAction = src->saHpiEventLogInfoOverflowAction;

    dst->saHpiEventLogInfoOverflowReset = src->saHpiEventLogInfoOverflowReset;

    dst->saHpiEventLogClear = src->saHpiEventLogClear;
...
saHpiEventLogInfoTable_extract_index( saHpiEventLogInfoTable_context * ctx, netsnmp_index * hdr )
{
        ...
        DEBUGMSGTL ((AGENT, "saHpiEventLogInfoTable_extract_index, called\n"));
...
saHpiEventLogInfoTable_create_row( netsnmp_index* hdr)
{
     ...
     */
    /**
     ctx->saHpiEventLogInfoTime = 0;
     ctx->saHpiEventLogInfoOverflowResetable = 0;
     ctx->saHpiEventLogInfoOverflowAction = 0;
     ctx->saHpiEventLogInfoOverflowReset = 0;
     ctx->saHpiEventLogClear = 0;
