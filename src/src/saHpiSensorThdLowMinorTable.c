 ...
 *
 * $Id: saHpiSensorThdLowMinorTable.c 5425 2005-11-14 21:48:21Z djudkovi $
 *
 *
...
oid saHpiSensorThdLowMinorTable_oid[] = { saHpiSensorThdLowMinorTable_TABLE_OID };
size_t saHpiSensorThdLowMinorTable_oid_len = OID_LENGTH(saHpiSensorThdLowMinorTable_oid);
				     ...
				     SaHpiSensorThresholdsT *sensor_thresholds)
{
	...
	DEBUGMSGTL ((AGENT, "populate_sen_thd_low_minor, called\n"));

	SaErrorT rv = SA_OK;
        int new_row = MIB_FALSE;

	oid sen_thd_low_minor_oid[SEN_THD_LOW_MINOR_IDX_NR];
	netsnmp_index sen_thd_low_minor_idx;
	saHpiSensorThdLowMinorTable_context *sen_thd_low_minor_ctx;

	/* check for NULL pointers */
	if (!rdr_entry) {
		DEBUGMSGTL ((AGENT, 
		"ERROR: populate_sen_thd_low_minor() passed NULL rdr_entry pointer\n"));
	...
	sen_thd_low_minor_oid[0] = get_domain_id(sessionid);
	/** Index saHpiResourceId is external */
	sen_thd_low_minor_oid[1] = rpt_entry->ResourceId;
	/** Index saHpiResourceIsHistorical is external */
	sen_thd_low_minor_oid[2] = MIB_FALSE;
	/** Index saHpiSensorNum */
	sen_thd_low_minor_oid[3] = rdr_entry->RdrTypeUnion.SensorRec.Num;
	/* assign the indices to the index */
	sen_thd_low_minor_idx.oids = (oid *) & sen_thd_low_minor_oid;

	/* See if Row exists. */
	sen_thd_low_minor_ctx = NULL;
	sen_thd_low_minor_ctx = CONTAINER_FIND(cb.container, &sen_thd_low_minor_idx);

	if (!sen_thd_low_minor_ctx) {
		// New entry. Add it
		sen_thd_low_minor_ctx = 
		saHpiSensorThdLowMinorTable_create_row(&sen_thd_low_minor_idx);
                new_row = MIB_TRUE;
	}
	if (!sen_thd_low_minor_ctx) {
		snmp_log (LOG_ERR, 
	...
	if (SAHPI_STM_LOW_MINOR & 
		...
		sen_thd_low_minor_ctx->saHpiSensorThdLowMinorIsWritable =
			MIB_TRUE;
	} else {
		sen_thd_low_minor_ctx->saHpiSensorThdLowMinorIsWritable =
			MIB_FALSE;
	...
	sen_thd_low_minor_ctx->saHpiSensorThdLowMinorType = 
		sensor_thresholds->LowCritical.Type + 1; 

        /** SaHpiSensorReadingValue = ASN_OCTET_STR */
	sen_thd_low_minor_ctx->saHpiSensorThdLowMinorValue_len =
		set_sensor_reading_value(&sensor_thresholds->LowMinor,
			sen_thd_low_minor_ctx->saHpiSensorThdLowMinorValue);

        /** TruthValue = ASN_INTEGER */
	    sen_thd_low_minor_ctx->saHpiSensorThdLowMinorNonLinear = 
 ...
 */
static int saHpiSensorThdLowMinorTable_cmp( const void *lhs, const void *rhs );
 ...
 */
static int
saHpiSensorThdLowMinorTable_cmp( const void *lhs, const void *rhs )
{
    saHpiSensorThdLowMinorTable_context *context_l =
        (saHpiSensorThdLowMinorTable_context *)lhs;
    saHpiSensorThdLowMinorTable_context *context_r =
        (saHpiSensorThdLowMinorTable_context *)rhs;
...
/************************************************************
 * Initializes the saHpiSensorThdLowMinorTable module
 */
void
init_saHpiSensorThdLowMinorTable(void)
{
	DEBUGMSGTL ((AGENT, "init_saHpiSensorThdLowMinorTable, called\n"));
     ...
     *
     * saHpiSensorThdLowMinorTable_context * new_row = create_row(index);
 ...
 */
static int saHpiSensorThdLowMinorTable_row_copy(saHpiSensorThdLowMinorTable_context * dst,
                         saHpiSensorThdLowMinorTable_context * src)
{
    if(!dst||!src)
        return 1;
    ...
    dst->saHpiSensorThdLowMinorIsReadable = src->saHpiSensorThdLowMinorIsReadable;

    dst->saHpiSensorThdLowMinorIsWritable = src->saHpiSensorThdLowMinorIsWritable;

    dst->saHpiSensorThdLowMinorType = src->saHpiSensorThdLowMinorType;

    memcpy( dst->saHpiSensorThdLowMinorValue, src->saHpiSensorThdLowMinorValue, src->saHpiSensorThdLowMinorValue_len );
    dst->saHpiSensorThdLowMinorValue_len = src->saHpiSensorThdLowMinorValue_len;
 ...
 */
int
saHpiSensorThdLowMinorTable_extract_index( saHpiSensorThdLowMinorTable_context * ctx, netsnmp_index * hdr )
{
    /*
...
{
	DEBUGMSGTL ((AGENT, "saHpiSensorThdLowMinorTable_can_activate, called\n"));
                        ...
                        netsnmp_request_group * rg)
{
	...
	DEBUGMSGTL ((AGENT, "saHpiSensorThdLowMinorTable_can_deactivate, called\n"));
                    ...
                    netsnmp_request_group * rg)
{
	...
	DEBUGMSGTL ((AGENT, "saHpiSensorThdLowMinorTable_can_delete, called\n"));

    /*
     * probably shouldn't delete a row that we can't
     * deactivate.
     */
    if(saHpiSensorThdLowMinorTable_can_deactivate(undo_ctx,row_ctx,rg) != 1)
        return 0;
 ...
 *
 * returns a newly allocated saHpiSensorThdLowMinorTable_context
 *   structure if the specified indexes are not illegal
 * returns NULL for errors or illegal index values.
 */
saHpiSensorThdLowMinorTable_context *
saHpiSensorThdLowMinorTable_create_row( netsnmp_index* hdr)
{
    saHpiSensorThdLowMinorTable_context * ctx =
        SNMP_MALLOC_TYPEDEF(saHpiSensorThdLowMinorTable_context);

    DEBUGMSGTL ((AGENT, "saHpiSensorThdLowMinorTable_create_row, called\n"));
 ...
 */
saHpiSensorThdLowMinorTable_context *
saHpiSensorThdLowMinorTable_duplicate_row( saHpiSensorThdLowMinorTable_context * row_ctx)
{
    saHpiSensorThdLowMinorTable_context * dup;

    DEBUGMSGTL ((AGENT, "saHpiSensorThdLowMinorTable_duplicate_row, called\n"));
 ...
 */
netsnmp_index * saHpiSensorThdLowMinorTable_delete_row( saHpiSensorThdLowMinorTable_context * ctx )
{
 ...
 */
void saHpiSensorThdLowMinorTable_set_reserve1( netsnmp_request_group *rg )
{
    saHpiSensorThdLowMinorTable_context *row_ctx =
...
//            (saHpiSensorThdLowMinorTable_context *)rg->undo_info;
    ...
    DEBUGMSGTL ((AGENT, "saHpiSensorThdLowMinorTable_set_reserve1, called\n"));
	...
	case COLUMN_SAHPISENSORTHDLOWMINORVALUE:
		...
		if (rc == SNMP_ERR_NOERROR ) {
			rc = check_sensor_reading_value(
				var->val_len, 
				row_ctx->saHpiSensorThdLowMinorType);
			if (rc != SNMP_ERR_NOERROR) {
				DEBUGMSGTL ((AGENT, 
				"COLUMN_SAHPISENSORTHDLOWMINORVALUE ERROR: %d\n", 
				rc));
			}
		}
		break;
        ...
        default: /** We shouldn't get here */
            ...
            snmp_log(LOG_ERR, "unknown column in "
                     "saHpiSensorThdLowMinorTable_set_reserve1\n");
        }
 ...
 * undo_info.
 */
void saHpiSensorThdLowMinorTable_set_commit( netsnmp_request_group *rg )
{
    netsnmp_variable_list *var;
//    saHpiSensorThdLowMinorTable_context *row_ctx = 
// 	(saHpiSensorThdLowMinorTable_context *)rg->existing_row;
...
// 	(saHpiSensorThdLowMinorTable_context *)rg->undo_info;
    ...
    DEBUGMSGTL ((AGENT, "saHpiSensorThdLowMinorTable_set_commit, called\n"));
 ...
 */
void saHpiSensorThdLowMinorTable_set_free( netsnmp_request_group *rg )
{
    netsnmp_variable_list *var;
//    saHpiSensorThdLowMinorTable_context *row_ctx = 
// 	(saHpiSensorThdLowMinorTable_context *)rg->existing_row;
...
// 	(saHpiSensorThdLowMinorTable_context *)rg->undo_info;
    ...
    DEBUGMSGTL ((AGENT, "saHpiSensorThdLowMinorTable_set_free, called\n"));
 ...
 * == 1).
 */
void saHpiSensorThdLowMinorTable_set_undo( netsnmp_request_group *rg )
{
    netsnmp_variable_list *var;
//    saHpiSensorThdLowMinorTable_context *row_ctx = 
// 	(saHpiSensorThdLowMinorTable_context *)rg->existing_row;
...
// 	(saHpiSensorThdLowMinorTable_context *)rg->undo_info;
    ...
    DEBUGMSGTL ((AGENT, "saHpiSensorThdLowMinorTable_set_undo, called\n"));
...
/************************************************************
 *
 * Initialize the saHpiSensorThdLowMinorTable table by defining its contents and how it's structured
 */
void
initialize_table_saHpiSensorThdLowMinorTable(void)
{
    netsnmp_table_registration_info *table_info;

    DEBUGMSGTL ((AGENT, "initialize_table_saHpiSensorThdLowMinorTable, called\n"));

    if(my_handler) {
        snmp_log(LOG_ERR, "initialize_table_saHpiSensorThdLowMinorTable_handler called again\n");
        return;
    }
    ...
    my_handler = netsnmp_create_handler_registration("saHpiSensorThdLowMinorTable",
                                             netsnmp_table_array_helper_handler,
                                             saHpiSensorThdLowMinorTable_oid,
                                             saHpiSensorThdLowMinorTable_oid_len,
                                             HANDLER_CAN_RWRITE);
            
    if (!my_handler || !table_info) {
        snmp_log(LOG_ERR, "malloc failed in "
                 "initialize_table_saHpiSensorThdLowMinorTable_handler\n");
    ...
    table_info->min_column = saHpiSensorThdLowMinorTable_COL_MIN;
    table_info->max_column = saHpiSensorThdLowMinorTable_COL_MAX;

    /***************************************************
     * registering the table with the master agent
     */
    cb.get_value = saHpiSensorThdLowMinorTable_get_value;
    cb.container = netsnmp_container_find("saHpiSensorThdLowMinorTable_primary:"
                                          "saHpiSensorThdLowMinorTable:"
                                          "table_container");

    netsnmp_container_add_index(cb.container,
                                netsnmp_container_find("saHpiSensorThdLowMinorTable_secondary:"
                                                       "saHpiSensorThdLowMinorTable:"
                                                       "table_container"));
    cb.container->next->compare = saHpiSensorThdLowMinorTable_cmp;


    cb.can_set = 1;

    cb.create_row = (UserRowMethod*)saHpiSensorThdLowMinorTable_create_row;

    cb.duplicate_row = (UserRowMethod*)saHpiSensorThdLowMinorTable_duplicate_row;
    cb.delete_row = (UserRowMethod*)saHpiSensorThdLowMinorTable_delete_row;
    cb.row_copy = (Netsnmp_User_Row_Operation *)saHpiSensorThdLowMinorTable_row_copy;

    cb.can_activate = (Netsnmp_User_Row_Action *)saHpiSensorThdLowMinorTable_can_activate;
    cb.can_deactivate = (Netsnmp_User_Row_Action *)saHpiSensorThdLowMinorTable_can_deactivate;
    cb.can_delete = (Netsnmp_User_Row_Action *)saHpiSensorThdLowMinorTable_can_delete;

    cb.set_reserve1 = saHpiSensorThdLowMinorTable_set_reserve1;
    cb.set_reserve2 = saHpiSensorThdLowMinorTable_set_reserve2;
    cb.set_action = saHpiSensorThdLowMinorTable_set_action;
    cb.set_commit = saHpiSensorThdLowMinorTable_set_commit;
    cb.set_free = saHpiSensorThdLowMinorTable_set_free;
    cb.set_undo = saHpiSensorThdLowMinorTable_set_undo;

    DEBUGMSGTL(("initialize_table_saHpiSensorThdLowMinorTable",
                "Registering table saHpiSensorThdLowMinorTable "
                "as a table array\n"));
...
/************************************************************
 * saHpiSensorThdLowMinorTable_get_value
 *
 ...
 */
int saHpiSensorThdLowMinorTable_get_value(
            netsnmp_request_info *request,
            netsnmp_index *item,
            netsnmp_table_request_info *table_info )
{
    netsnmp_variable_list *var = request->requestvb;
    saHpiSensorThdLowMinorTable_context *context = (saHpiSensorThdLowMinorTable_context *)item;

    DEBUGMSGTL ((AGENT, "saHpiSensorThdLowMinorTable_get_value, called\n"));
...
/************************************************************
 * saHpiSensorThdLowMinorTable_get_by_idx
 */
const saHpiSensorThdLowMinorTable_context *
saHpiSensorThdLowMinorTable_get_by_idx(netsnmp_index * hdr)
{
	DEBUGMSGTL ((AGENT, "saHpiSensorThdLowMinorTable_get_by_idx, called\n"));

    return (const saHpiSensorThdLowMinorTable_context *)
