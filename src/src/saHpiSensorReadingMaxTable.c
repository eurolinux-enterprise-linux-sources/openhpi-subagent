 ...
 *
 * $Id: saHpiSensorReadingMaxTable.c 5424 2005-11-14 21:35:39Z djudkovi $
 *
 *
 ...
 * @return 
 */
SaErrorT clear_sensor_max(SaHpiDomainIdT domainId, SaHpiResourceIdT resourceId)

{
        SaErrorT rv = SA_OK;
        netsnmp_index *row_idx;
        saHpiSensorReadingMaxTable_context *sen_max_ctx;

	DEBUGMSGTL ((AGENT, "clear_sensor_max, called\n"));	
	...
	DEBUGMSGTL ((AGENT, "           resourceId [%d]\n", resourceId));
        ...
        if (row_idx) //At least one entry was found.
        {
                do {
                        ...
                        sen_max_ctx = CONTAINER_FIND(cb.container, row_idx);
                        ...
                        if ((sen_max_ctx->index.oids[saHpiSenMaxDomainId_INDEX] ==
                             domainId) &&

                            (sen_max_ctx->index.oids[saHpiSenMaxResourceId_INDEX] ==
                             resourceId)) {

                                /* all conditions met remove row */
                                CONTAINER_REMOVE (cb.container, sen_max_ctx);
                                saHpiSensorReadingMaxTable_delete_row (sen_max_ctx);
                                DEBUGMSGTL ((AGENT, "clear_sensor_max: "
                                                    "found row: removing\n"));
