 ...
 *
 * $Id: saHpiSensorThdNegHysteresisTable.c 5425 2005-11-14 21:48:21Z djudkovi $
 *
 *
...

{
        ...
        saHpiSensorThdNegHysteresisTable_context *thd_ctx;

	DEBUGMSGTL ((AGENT, "clear_sen_thd_neg_hys, called\n"));	
	...
	DEBUGMSGTL ((AGENT, "           resourceId [%d]\n", resourceId));
        ...
        if (row_idx) //At least one entry was found.
        {
                do {
                        ...
                        if ((thd_ctx->index.oids[saHpiThdNegHysDomainId_INDEX] ==
                             domainId) &&

                            (thd_ctx->index.oids[saHpiThdNegHysResourceEntryId_INDEX] ==
                             resourceId)) {

                                /* all conditions met remove row */
                                CONTAINER_REMOVE (cb.container, thd_ctx);
                                saHpiSensorThdNegHysteresisTable_delete_row (thd_ctx);
                                DEBUGMSGTL ((AGENT, "clear_sen_thd_neg_hys: "
                                                    "found row: removing\n"));
...
int set_table_sen_thds_neg_hys (saHpiSensorThdNegHysteresisTable_context *row_ctx)
{
        ...
        session_id = get_session_id(row_ctx->index.oids[saHpiThdNegHysDomainId_INDEX]);
        resource_id = row_ctx->index.oids[saHpiThdNegHysResourceEntryId_INDEX];
        sensor_num = row_ctx->index.oids[saHpiSensorThdNegHysNum_INDEX];
 ...
 */
static int saHpiSensorThdNegHysteresisTable_cmp( const void *lhs, const void *rhs );
...
/************************************************************
 * Initializes the saHpiSensorThdNegHysteresisTable module
 */
void
init_saHpiSensorThdNegHysteresisTable(void)
{
 ...
 *
 * returns a newly allocated saHpiSensorThdNegHysteresisTable_context
...
/************************************************************
 *
 * Initialize the saHpiSensorThdNegHysteresisTable table by defining its contents and how it's structured
 */
void
